...
class Coordination_GameRunner(Runner):
    ...
    def __init__(self, config):
        super(Coordination_GameRunner, self).__init__(config)
    ...
    def run(self):
        ...
        game = CoordinationGame(n, 0.1, mu, gamma = 0.95, device = device)
        self.trainer.policy.game = game
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                # Sample actions
                values, actions, action_log_probs, rnn_states, rnn_states_critic, actions_env, father_actions, edge_noise, permutation_noise = self.collect(step)  ## (1,4,1)*3, (1,4,1,64)*2, (1,4,5)
               
                # Obser reward and next obs
                obs, rewards, dones, infos = self.envs.step(actions_env)
                # (1,4,24) (1,4,1) (1,4) (1,4)
            ...
            if episode % self.eval_interval == 0 and self.use_eval:
                _, avg_open_ratios = self.eval()
                poa = game.get_poa(self.trainer.policy).item()
                #eval_episodic_rewards[total_num_steps] = str(avg_episodic_rewards)
                eval_episodic_poa[episode] = str(poa)
                eval_episodic_open_ratios[episode] = avg_open_ratios
        
        if not os.path.exists(self.save_dir+ '/eval_episodic_poa/'):
            os.makedirs(self.save_dir+ '/eval_episodic_poa/')
        if not os.path.exists(self.save_dir+ '/eval_episodic_open_ratios/'):
            os.makedirs(self.save_dir+ '/eval_episodic_open_ratios/')

        with open(self.save_dir + '/eval_episodic_poa/'+str(self.all_args.seed)+'.json', 'w') as fp:
            json.dump(eval_episodic_poa, fp)
