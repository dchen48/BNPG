...
class ACTLayer(nn.Module):
        ...
        for step in range(self.num_agents):
            ...
            for agent_id in range(self.num_agents):
                if indicies[agent_id] == []:
                    continue
                masks_i = torch.tensor(masks[agent_id]).cuda()
                masks_i = masks_i.unsqueeze(-1).repeat(1,1, self.action_dim).view(len(indicies[agent_id]), -1)
                ...
                father_action = father_action.view(len(indicies[agent_id]), -1)
  
                father_action = father_action.detach() * masks_i
                father_action = father_action.view(len(indicies[agent_id]), -1)
            
                x_i = torch.cat((x[indicies[agent_id],agent_id,:], father_action.cuda()), dim = -1)
                action_logit = self.action_out_list[agent_id](x_i, available_actions)
    ...
    def evaluate_actions(self, x, agent_id_batch, father_action, action, available_actions=None, active_masks=None):
        """
