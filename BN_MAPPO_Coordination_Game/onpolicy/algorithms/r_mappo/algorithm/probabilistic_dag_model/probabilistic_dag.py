...
class ProbabilisticDAG(nn.Module):
    ...
    def __init__(self, n_nodes, input_dim, hidden_dim, temperature=1.0, hard=True, noise_factor=1.0, edge_net_type = 'deep_set', device = 'cpu'):
        ...
        #egde network
        if self.edge_net_type == 'deep_set':
            self.edge_net = DeepSet(self.input_dim, self.n_nodes, self.n_nodes, self.hidden_dim)
        else:
...
class DummyDAG(nn.Module):
    ...
    def __init__(self, n_nodes, graph_type = 'dynamic', threshold = 1, device = 'cpu'):
        """Base Class for Probabilistic DAG Generator based on topological order sampling
