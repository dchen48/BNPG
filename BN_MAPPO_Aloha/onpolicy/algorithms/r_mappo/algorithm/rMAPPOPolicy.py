...
class R_MAPPOPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, device=torch.device("cpu")):
        ...
        if self.graph_type == 'dynamic':
            self.dag_net = ProbabilisticDAG(self.args.num_agents, self.obs_space.shape[0]*self.args.num_agents, args.hidden_size, temperature=1.0, hard=True, noise_factor=1.0, edge_net_type = args.edge_net_type, device = self.device)
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, last_actions, available_actions=None,
        ...
        inputs_graph = obs_.view(self.n_rollout_threads, -1)
        

        P, U, G_s, edge_noise, permutation_noise =  self.dag_net.sample(inputs_graph)
    ...
    def get_eval_actions(self, obs, rnn_states_actor, masks, last_actions, available_actions=None,
        ...
        P, U, G_s, edge_noise, permutation_noise =  self.dag_net.sample(inputs_graph, test = True)
    ...
    def evaluate_actions(self, cent_obs, obs, agent_id_batch, graph_obs_batch, father_action, last_actions_batch, graph_last_actions_batch, edge_noise_batch, permutation_noise_batch, rnn_states_actor, graph_rnn_states_actor, rnn_states_critic, action, masks, graph_masks,
        ...
        inputs_graph= inputs_graph.view(len(inputs_graph), -1)
        ...
        for i in range(len(father_action)):
            ...
            father_oa = torch.cat((graph_obs_batch[i], graph_father_actions_i[i]), dim=1)
            mask_i = mask_i.view(-1, 1).repeat(1, father_oa.shape[-1])
            masked_father_action.append(father_oa * mask_i)
